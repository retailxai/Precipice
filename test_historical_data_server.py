#!/usr/bin/env python3
"""
Test RetailXAI with historical data from the production server.
This script will collect data from the last 2 months from the production server
and run the full analysis pipeline locally.
"""
...
def test_production_server_connection():
    """Test connection to production server."""
    print("🔗 Testing connection to production server...")
    
    try:
        response = requests.get(f"{PRODUCTION_SERVER}/api/health", timeout=10)
        response.raise_for_status()
        health_data = response.json()
        
        if health_data.get('database_connected'):
            print("✅ Production server is online and healthy")
            return True
        else:
            print("⚠️  Production server is online but database is not connected")
            return False
            
    except Exception as e:
        print(f"❌ Cannot connect to production server: {e}")
        return False

def main():
    """Main function to run historical data test."""
    print("🚀 RetailXAI Historical Data Test (Production Server)")
    print("=" * 60)
    
    # Test production server connection
    if not test_production_server_connection():
        print("❌ Cannot proceed without production server connection")
        return
    ...
    print(f"🖥️  Production Server: {PRODUCTION_SERVER}")
    print("\n✅ Historical data test completed!")
    print("\n🌐 You can now view the results at: http://localhost:5000")
    print("📊 Or generate static site for GitHub Pages: python3 generate_static_site.py")
